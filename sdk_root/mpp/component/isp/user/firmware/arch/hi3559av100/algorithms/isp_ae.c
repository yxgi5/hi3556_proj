...
static HI_S32 AeGetDCFInfo(VI_PIPE ViPipe, ISP_AE_RESULT_S *pstAeResult)
{
    ...
    pstIspUpdateInfo = pstIspCtx->stUpdateInfoCtrl.pstIspUpdateInfo;
...
static HI_S32 AeGetFrameInfo(VI_PIPE ViPipe, ISP_AE_RESULT_S *pstAeResult)
{
    ...
    ISP_GET_CTX(ViPipe, pstIspCtx);
    pstIspFrame = pstIspCtx->stFrameInfoCtrl.pstIspFrame;
    ...
    {
        pstIspFrame->au32Ratio[0] = (pstAeResult->u32IntTime[1] << 2) / DIV_0_TO_1(pstAeResult->u32IntTime[0] >> 4);
        pstIspFrame->au32Ratio[1] = (pstAeResult->u32IntTime[2] << 2) / DIV_0_TO_1(pstAeResult->u32IntTime[1] >> 4);
        pstIspFrame->au32Ratio[2] = 64;
    }
    else if (IS_4to1_WDR_MODE(pstIspCtx->u8SnsWDRMode))
    {
        pstIspFrame->au32Ratio[0] = (pstAeResult->u32IntTime[1] << 2) / DIV_0_TO_1(pstAeResult->u32IntTime[0] >> 4);
        pstIspFrame->au32Ratio[1] = (pstAeResult->u32IntTime[2] << 2) / DIV_0_TO_1(pstAeResult->u32IntTime[1] >> 4);
        pstIspFrame->au32Ratio[2] = (pstAeResult->u32IntTime[3] << 2) / DIV_0_TO_1(pstAeResult->u32IntTime[2] >> 4);
    }
    else
    {
...
static HI_VOID AeUpdateLinkage(VI_PIPE ViPipe, ISP_AE_RESULT_S *pstAeResult, ISP_LINKAGE_S *pstLinkage)
{
    ...
    {
        pstLinkage->u32Inttime = pstAeResult->u32IntTime[2] >> 4;
        pstLinkage->u32ExpRatio = (pstAeResult->u32IntTime[2] << 2) / DIV_0_TO_1(pstAeResult->u32IntTime[0] >> 4);
        pstLinkage->au32ExpRatio[0] = (pstAeResult->u32IntTime[1] << 2) / DIV_0_TO_1(pstAeResult->u32IntTime[0] >> 4);
        pstLinkage->au32ExpRatio[1] = (pstAeResult->u32IntTime[2] << 2) / DIV_0_TO_1(pstAeResult->u32IntTime[1] >> 4);
        hi_ext_system_actual_wdr_exposure_ratio_write(ViPipe, 0, pstLinkage->au32ExpRatio[0]);
        hi_ext_system_actual_wdr_exposure_ratio_write(ViPipe, 1, pstLinkage->au32ExpRatio[1]);
        hi_ext_system_actual_wdr_exposure_ratio_write(ViPipe, 2, 64);
    }
    else if (IS_4to1_WDR_MODE(pstIspCtx->u8SnsWDRMode))
    {
        pstLinkage->u32Inttime = pstAeResult->u32IntTime[3] >> 4;
        pstLinkage->u32ExpRatio = (pstAeResult->u32IntTime[3] << 2) / DIV_0_TO_1(pstAeResult->u32IntTime[0] >> 4);
        pstLinkage->au32ExpRatio[0] = (pstAeResult->u32IntTime[1] << 2) / DIV_0_TO_1(pstAeResult->u32IntTime[0] >> 4);
        pstLinkage->au32ExpRatio[1] = (pstAeResult->u32IntTime[2] << 2) / DIV_0_TO_1(pstAeResult->u32IntTime[1] >> 4);
        pstLinkage->au32ExpRatio[2] = (pstAeResult->u32IntTime[3] << 2) / DIV_0_TO_1(pstAeResult->u32IntTime[2] >> 4);
        hi_ext_system_actual_wdr_exposure_ratio_write(ViPipe, 0, pstLinkage->au32ExpRatio[0]);
        hi_ext_system_actual_wdr_exposure_ratio_write(ViPipe, 1, pstLinkage->au32ExpRatio[1]);
        hi_ext_system_actual_wdr_exposure_ratio_write(ViPipe, 2, pstLinkage->au32ExpRatio[2]);
    }
    else
    {
                 ...
                 HI_VOID *pRegCfg, HI_S32 s32Rsv)
{
    ...
    if (HI_TRUE == pstIspCtx->stStitchAttr.bStitchEnable)
    {
        ...
        {
            stAeInfo.pstFEAeStat1 = &pIspStatInfo->stStitchStat.stFEAeStat1;
            stAeInfo.pstFEAeStat2 = &pIspStatInfo->stStitchStat.stFEAeStat2;
            stAeInfo.pstBEAeStat1 = &pIspStatInfo->stStitchStat.stBEAeStat1;
            stAeInfo.pstBEAeStat2 = &pIspStatInfo->stStitchStat.stBEAeStat2;
            stAeInfo.pstFEAeStiStat = &pIspStatInfo->stStitchStat.stFEAeStat3;
            stAeInfo.pstBEAeStiStat = &pIspStatInfo->stStitchStat.stBEAeStat3;
        ...
        {
            memcpy(&stAeResult, &g_astIspCtx[s8StitchMainPipe].stAeResult, sizeof(ISP_AE_RESULT_S));
        }
    }
    else
    {
