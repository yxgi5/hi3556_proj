...
HI_S32 ISP_UpdateInfo(VI_PIPE ViPipe)
{
    ...
    {
        ISP_TRACE(HI_DBG_ERR, "pipe:%d the isp update info buf hasn't init!\n", ViPipe);
        return HI_FAILURE;
    }

    for (i = ISP_MAX_UPDATEINFO_BUF_NUM - 1; i >= 1; i--)
    {
        memcpy(pstIspCtx->stUpdateInfoCtrl.pstIspUpdateInfo + i, pstIspCtx->stUpdateInfoCtrl.pstIspUpdateInfo + i - 1, sizeof(ISP_DCF_UPDATE_INFO_S));
    }
    ...
    for (i = 0; i < ISP_MAX_ALGS_NUM; i++)
    {
        if (pstIspCtx->astAlgs[i].bUsed)
        {
            ...
            {
                pstIspCtx->astAlgs[i].stAlgFunc.pfn_alg_ctrl(ViPipe, ISP_UPDATE_INFO_GET, pstIspCtx->stUpdateInfoCtrl.pstIspUpdateInfo);
            }
        }
    }
...
HI_S32 ISP_UpdateFrameInfo(VI_PIPE ViPipe)
{
    ...
    {
        ISP_TRACE(HI_DBG_ERR, "pipe:%d the Isp Frame Info hasn't init!\n", ViPipe);
        return HI_FAILURE;
    }

    for (i = ISP_MAX_FRAMEINFO_BUF_NUM - 1; i >= 1; i--)
    {
        memcpy(pstIspCtx->stFrameInfoCtrl.pstIspFrame + i, pstIspCtx->stFrameInfoCtrl.pstIspFrame + i - 1, sizeof(ISP_FRAME_INFO_S));
    }
    ...
    for (i = 0; i < ISP_MAX_ALGS_NUM; i++)
    {
        if (pstIspCtx->astAlgs[i].bUsed)
        {
            if (HI_NULL != pstIspCtx->astAlgs[i].stAlgFunc.pfn_alg_ctrl)
            {
                pstIspCtx->astAlgs[i].stAlgFunc.pfn_alg_ctrl(ViPipe, ISP_FRAMEINFO_GET, pstIspCtx->stFrameInfoCtrl.pstIspFrame);
            }
        }
    }
